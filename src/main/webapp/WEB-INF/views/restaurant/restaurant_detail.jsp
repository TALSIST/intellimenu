...
<body>
	<div class="container">
		...
		<section class="main-section contact" id="contact">
			<div class="row">
				<div
					style="float: left; margin-left: 100px; margin-bottom: 50px; width: 40%;">
					<div style="font-size: 32px; color: black; margin: auto;">
						<img src="/img/spaguetti.png" width="45px">&nbsp;${vo.name }<span
							style="font-size: 25px; color: Orange;">&nbsp;&nbsp;${vo.score }</span>
					</div>
					<div style="margin: auto;">
						<hr>
						<span> 주소:</span><span style="color: black;"> ${vo.sigun }
							${vo.address2 }</span>
						<p>
							<c:if test="${vo.tel!=null }">
								<span>전화번호:</span>
								<span style="color: black;"> ${vo.tel }</span>
								<p>
							</c:if>
							<c:if test="${vo.category!=null }">
								<span>음식종류:</span>
								<span style="color: black;"> ${vo.category }</span>
								<p>
							</c:if>
							<c:if test="${vo.parking!=null }">
								<span>주차:</span>
								<span style="color: black;"> ${vo.parking }</span>
								<p>
							</c:if>
							<c:if test="${vo.busihour!=null }">
								<span>영업시간:</span>
								<span style="color: black;"> ${vo.busihour }</span>
								<p>
							</c:if>
							<c:if test="${vo.holiday!=null }">
								<span>휴일:</span>
								<span style="color: black;"> ${vo.holiday }</span>
								<p>
							</c:if>
					</div>
				</div>
				<div style="float: right; margin-bottom: 50px; width: 40%;">
					<c:if test="${vo.img_ori!=null&&vo.img_new==null }">
						<img src="${vo.img_ori}" width="350px" height="350px"
					...
					<c:if test="${vo.img_ori!=null&&vo.img_new!=null}">
						...
						<img src="/resources/restaurant/2017/${vo.img_new}" width="350px"
							height="350px" style="border-radius: 20%;">
					</c:if>
			...
			<div class="container">
				<div
					style="font-size: 25px; color: black; margin: auto; text-align: center;">
				...
				<form>
					...
					평점 : <input type="radio" name="scores" value="1"> 1 <input
						type="radio" name="scores" value="2"> 2 <input
						type="radio" name="scores" value="3" checked> 3 <input
						type="radio" name="scores" value="4"> 4 <input
						type="radio" name="scores" value="5"> 5
					<div>
						<label for="uploadedImages"> <img
							src="http://recipe.ezmember.co.kr/img/pic_none3.gif"
							class="fileDrop"
							style="height: 100px; width: 100px; border: 1px solid #a0a0a0" />
						</label> <input multiple onChange="readURL(this.files);"
							id="uploadedImages" name="pictures[]" class="fileDrop"
			...
			$.each(files,
							function(value) {
								...
								reader.onload = function(e) {
									...
									$('#up_images')
											.prepend(
													'<img class=\'all_images\' id='
															+ id
															+ ' src='
															+ e.target.result
															+ ' style="width:100px;height=100px;" data-index='
															+ number
							...
							for (var i = 0; i < list.length; i++) {
								if (list[i].report < 5) {
									...
									output += "<span> 작성자ID : "
											+ list[i].user_id + "  </span>";
									output += "<span> ( "
											+ changeDate(list[i].regdate)
											+ ") </span>";
									output += "<span> 평점 : " + list[i].score
											+ "  </span>";
									output += "<button onClick=\"report("
											+ list[i].id
											+ ")\">신고</button><br>";
									output += "<span>" + list[i].reply
											+ "</span>";
									output += "<div class=\"w3-row-padding\">";
									for ( var j in imgs) {
										output += "<img src=\"/resources/restaurant/2017/"
												+ imgs[j] + "\" hspace=\"5\"";
